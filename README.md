...
**To add a new page**: copy the `<head>` + `shared.css` link + slot divs + `renderChrome('key')` call from `contact.html`, then add `['your-page.html', 'Label', 'key']` to the `links` array in `shared.js` (top of file) so it appears in nav on every page.
